...
// }
let sql = "INSERT INTO parking_lot(pos_num, free_num, master_id, latitude, longitude, chart_id, address, adcode, remark, create_time)" +
    " VALUES(?,?,?,?,?,?,?,?,?,?)";
        ...
        function insertParkingUnit() {
            ...
            myHttp.get(options, res => {
                if(res.code == '200') {
                    ...
                    let date = new Date();
                    connection.query(sql, [0, 0, 1, latitude, longitude, 0, address, adcode, remark, date], (err, result)=>{
                        if(result) {
                            console.log('success');
                        }else {
